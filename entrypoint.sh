...
## ── ZK-DKG files: auto-download if missing ──
ZK_BASE_URL="https://zkstorage.blob.core.windows.net/zk-blob"
mkdir -p "$DATADIR/r1cs" "$DATADIR/pk"

for name in one_message two_message seven_message; do
  if [ ! -f "$DATADIR/r1cs/${name}.ccs" ]; then
    echo "⬇️  Downloading ${name}.ccs ..."
    wget -q --show-progress -O "$DATADIR/r1cs/${name}.ccs" "$ZK_BASE_URL/${name}.ccs"
  fi
  if [ ! -f "$DATADIR/pk/${name}.pk" ]; then
    echo "⬇️  Downloading ${name}.pk ..."
    wget -q --show-progress -O "$DATADIR/pk/${name}.pk" "$ZK_BASE_URL/${name}.pk"
  fi
done
